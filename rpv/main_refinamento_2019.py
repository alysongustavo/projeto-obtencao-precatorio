...
filePrecatorio = open("../dados/2019/rpv/resultados/precatorios_cancelados_para_refinar.txt", 'r')
...
def escrever_file_precatorio_cancelado_refinado(rpv):
    f = open('../dados/2019/rpv/resultados/pb_valores_maior_que_mil/precatorios_cancelados_pb_maior_que_mil_2019.txt', 'a')
    f.write(rpv + '\n')
    f.close()

def escrever_file_precatorio_cancelado_refinado_nao_pb_ou_menor_que_mil(rpv):
        f = open(
            '../dados/2019/rpv/resultados/pb_valores_maior_que_mil/precatorios_cancelados_nao_pb_ou_menor_que_mil_reais.txt', 'a')
        f.write(rpv + '\n')
        f.close()

def escrever_file_falha_request(rpv):
    f = open('../dados/2019/rpv/resultados/pb_valores_maior_que_mil/arquivo_falha_request.txt', 'a')
    f.write(rpv + '\n')
    f.close()
...
def ler_file_precatorio_cancelado_refinado():
   LISTA_RPV_CANCELADO =[]
   filePrecatorio = open(
       "../dados/2019/rpv/resultados/pb_valores_maior_que_mil/precatorios_cancelados_pb_maior_que_mil_2019.txt", 'r')
   Lines = filePrecatorio.readlines()
   for line in Lines:
...
def ler_file_precatorio_cancelado_refinado_nao_pb_ou_menor_que_mil():
   LISTA_RPV_CANCELADO =[]
   filePrecatorio = open(
       "../dados/2019/rpv/resultados/pb_valores_maior_que_mil/precatorios_cancelados_nao_pb_ou_menor_que_mil_reais.txt", 'r')
   Lines = filePrecatorio.readlines()
   for line in Lines:
       LISTA_RPV_CANCELADO.append(line.strip())
...
def ler_arquivo_falha_request():
   ...
   filePrecatorio = open("../dados/2019/rpv/resultados/pb_valores_maior_que_mil/arquivo_falha_request.txt", 'r')
   Lines = filePrecatorio.readlines()
   for line in Lines:
...
async def make_account():
    url = TRF5_URL
    f = open('../dados/2019/rpv/resultados/pb_valores_maior_que_mil/arquivo_falha_request.txt', 'w')
    f.close()
...
async def do_post(session, url, rpv):
    ...
    try:
        ...
        if (rpv in ler_file_precatorio_cancelado_refinado() or rpv in ler_file_precatorio_cancelado_refinado_nao_pb_ou_menor_que_mil()):
            ...
            elif (rpvPB == rpv and  int(valorRemovendoPonto if type(valorRemovendoPonto) == int else 0) >= 1000):
